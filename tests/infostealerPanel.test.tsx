...
import type { LookupResponse, HudsonRockData, HudsonRockStealer, SourceResult } from "@/lib/types";
...
const data = (hudsonRock: SourceResult<HudsonRockData>): LookupResponse =>
  ({ sources: { hudsonRock } }) as unknown as LookupResponse;

describe("<InfostealerPanel>", () => {
  it("shows a rate-limit message when Hudson Rock is throttled", () => {
    render(<InfostealerPanel data={data({ ok: false, error: "RATE_LIMITED" })} />);
    expect(screen.getByText(/rate-limited/i)).toBeTruthy();
  });

  it("shows a generic failure with the error, and an 'unknown' fallback", () => {
    const { unmount } = render(<InfostealerPanel data={data({ ok: false, error: "boom" })} />);
    expect(screen.getByText(/check failed: boom/i)).toBeTruthy();
    unmount();
    render(<InfostealerPanel data={data({ ok: false } as SourceResult<HudsonRockData>)} />);
    expect(screen.getByText(/check failed: unknown/i)).toBeTruthy();
  });

  it("renders the CLEAN state when there are no infections", () => {
    render(<InfostealerPanel data={data({ ok: true, data: { total: 0, stealers: [] } })} />);
  ...
  it("renders a single infection with singular wording and every stealer field", () => {
    render(<InfostealerPanel data={data({ ok: true, data: { total: 1, stealers: [stealer({
  ...
  it("pluralises multiple infections and tolerates a stealer with no detail fields", () => {
    render(<InfostealerPanel data={data({ ok: true, data: { total: 2, stealers: [stealer(), stealer()] } })} />);
  ...
  it("handles a positive total that carries no stealer detail rows", () => {
    render(<InfostealerPanel data={data({ ok: true, data: { total: 3, stealers: [] } })} />);
