...
import { bannerPlain, bannerAnsi, bannerWidth } from "@/lib/brand/banner";
...
describe("committed terminal banner is current", () => {
  ...
  it("holds every coloured line the module renders today", () => {
    for (const line of bannerAnsi(stats)) {
      expect(sh(), FIX).toContain(`printf '%s\\n' '${line}'`);
    }
  });

  it("switches on NO_COLOR and on a non-TTY stdout", () => {
    // Piping the launcher's output into a log file should not fill it with
    // escape codes.
    expect(sh()).toContain('if [ -n "${NO_COLOR:-}" ] || [ ! -t 1 ]; then');
  });
