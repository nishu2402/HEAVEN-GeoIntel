...
- **A release banner for the GitHub release page and the social preview.**
  `docs/assets/heaven-release-banner.png` (1280x500 at 2x) carries the mark, the
  wordmark, the four-stage collect/enrich/correlate/report pipeline and six
  counts read off this release: 3397 tests, 100% coverage, 7 identifier types,
  34 sources, 11 modes, 30 API operations. It is generated by
  `scripts/generate-release-banner.mjs` (`npm run brand:release`, also chained
  into `npm run brand`), which reads the geometry from `src/lib/brand/logo.ts`
  and the version from `src/lib/version.ts` so neither can drift from the app.
  It is a release and social-preview asset, deliberately not embedded in the
  README, which already opens with the animated poster.
...
- **The README's lead screenshot showed an empty input box, not a result.** The
  capture script scrolled to the panel containing "THREAT SCORE", a string this
  codebase has never had (the panel reads INTELLIGENCE SCORE), and a miss simply
  left the page at the top, so the first image under "Screenshots" was the form
  before anyone had searched anything while its caption described a scored
  number. The bulk shot failed the same way one control later: it clicked a
  button matching `RUN BULK`, which is now just `Run`, so it captured five
  numbers pasted into a box above no table at all. Neither failure could be seen
  from a green build, because screenshots are not test-gated. Both lookups now
  go through helpers that throw and name what they could not find, the bulk shot
  waits for its five rows instead of sleeping for 1.2 seconds, and all seven
  PNGs were re-captured. Two captions that had drifted from the interface were
  rewritten with them: bulk takes five hundred rows of any identifier rather
  than twenty-five numbers, and the phone shot no longer claims to show the mode
  bar it does not include.

- **Screenshots carried the dev server's own badge and the photographer's API
  key state.** Every committed PNG had Next's floating dev-tools button burned
  into the bottom-left corner, and the AI Analysis shot announced "Google Gemini
  is ready (key saved on this machine)", which is a fact about the machine that
  took the picture and not about the tool anyone else installs. The script hides
  the dev overlay, and the documented way to run it points `HV_DATA_DIR` at a
...
- **Light theme: controls rendered between panels were invisible.** The theme was
  ...
  accents (5.6 to 6.2:1), and the components that sit outside a panel now use it.
...
- **Light theme: panels were paler than the dark theme's, so greys failed on one
  ...
  `#222734` against the dark theme's `#090d18`. Mid-greys landed at 4.0 to 4.2:1 in
...
- The audit record's §3.1 keeps its original "Residual, disclosed" paragraph as the
  record of what was true at audit time, with the closure written underneath it rather
  than over it. §9's open-items list marks the chain closed and points back at §3.1.
...
- **Change tracking, "re-run this and tell me what moved".** Pinning a lookup to a
  ...
  Verified live: a re-run reported `subdomains 1 → 4`, and DNSSEC newly `signed`.
...
- **User-friendliness overhaul** (no data/accuracy logic touched):
  ...
  - **Keyboard shortcuts.** `1` to `8` switch mode, `/` focuses the input; a **`?` help
...
- **Carrier lookup returned the wrong operator for MTN South Africa**
  ...
  `MCC-MNC` key matches `MCC(3)-MNC(2 to 3 digits)`.
...
- **Disclosed, not fixed:** the full tree still reports 9 **dev-only** advisories, all
  ...
  `eslint-config-next`'s own plugins. No fixed 1.x to 4.x of `brace-expansion` exists,
...
- **IP intelligence** (`/api/ip-lookup`): geolocation, ASN/ISP, reverse DNS, and
  VPN/proxy/hosting/mobile risk flags via ip-api, plus a 0 to 100 IP-risk score and
...
- **Action-centre empty states.** When an API key is missing, the breach
  and identity panels now show 8 to 10 free, no-key lookup buttons instead of
